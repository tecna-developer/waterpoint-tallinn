...
function mountMap(slot) {
  ...
  if (!map) {
    ...
    markerLayer = L.markerClusterGroup({
      ...
      // Кластеры перестраиваются мгновенно, без анимации разлёта. Плагин анимирует
      // перестроение через requestAnimationFrame, а кадровый цикл в этом приложении
      // уже дважды оказывался ненадёжным (тайлы карты, анимация шторки) — и на телефоне
      // это дало кластер «56» поверх карты уровня улиц: для 78 кранов такое число
      // соответствует зуму 7–8, то есть кластеры остались от вида на весь регион, пока
      // тайлы уехали на 15. Воспроизводилось отдалением и быстрым приближением обратно.
      // Плавность разлёта не стоит того, чтобы счётчики врали о содержимом карты.
      animate: false,
    ...
    // iOS Safari масштабирует двумя пальцами саму страницу, игнорируя `touch-action`,
    // который Leaflet ставит на контейнер. На устройстве это выглядело так: щипок по
    // карте увеличивал весь интерфейс — нижняя навигация обрезана, поиск и фильтры
    // уехали за экран, вернуть можно только обратным щипком. Гасим жест ТОЛЬКО над
    // картой, где у щипка есть собственный смысл (зум карты); на списке, карточке и
    // настройках масштабирование страницы обязано остаться рабочим — это требование
    // доступности (WCAG 1.4.4), и запрещать его глобально нельзя.
    // passive: false обязателен, иначе preventDefault() игнорируется.
    ['gesturestart', 'gesturechange', 'gestureend'].forEach(ev =>
      mapEl.addEventListener(ev, e => e.preventDefault(), { passive: false }));
    map.on('click', e => {
      if (ui.pickMode) {
...
// Радиус кластеризации зависит от зума. Единым числом хорошо не сделать: на общем виде
// нужна плотная группировка, иначе центр города превращается в кашу, а вблизи наоборот —
// человеку нужны сами точки, к которым он собирается идти, а не кружки со счётчиками.
...
function clusterRadius(zoom) {
  ...
  return 12;                   // 15 и ближе: только то, что и так стоит вплотную
}
